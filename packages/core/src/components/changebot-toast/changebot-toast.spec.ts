...
describe('changebot-toast', () => {
  ...
  describe('preview mode', () => {
    it('shows toast immediately with preview prop', async () => {
      const page = await newSpecPage({
        components: [ChangebotToast],
        html: '<changebot-toast preview="true"></changebot-toast>',
      });
    ...
    it('displays preview content', async () => {
      ...
      expect(content.textContent).toContain('preview mode');
    });
    ...
    it('does not require a provider in preview mode', async () => {
      ...
      // But toast should still be visible
      expect(component.isVisible).toBe(true);
    });

    it('respects position prop in preview mode', async () => {
      const page = await newSpecPage({
        components: [ChangebotToast],
        html: '<changebot-toast preview="true" position="top-left"></changebot-toast>',
      });
    ...
    it('respects theme prop in preview mode', async () => {
      const page = await newSpecPage({
        components: [ChangebotToast],
        html: '<changebot-toast preview="true" theme="dracula"></changebot-toast>',
      });

      const toast = page.root.shadowRoot.querySelector('.toast');
      expect(toast).toHaveClass('theme--dracula');
    });
    ...
    it('dismisses without calling services in preview mode', async () => {
      ...
      const mockMarkToastViewed = jest.fn();
      component.services = {
        highlight: { markToastViewed: mockMarkToastViewed }
      };
      ...
      // Should not call markToastViewed in preview mode
      expect(mockMarkToastViewed).not.toHaveBeenCalled();
      expect(component.isVisible).toBe(false);
    });
  });
});
